...
function renderEChart(labels, counts, labelTitle) {
    ...
    const option = {
        ...
        series: [{ data: counts, type: 'bar', itemStyle: { color: 'rgba(255, 99, 132, 0.5)' } }]
    };
...
async function callGemini() {
    ...
    const contents = [
        {
            role: "user",
            parts: [{
                text: `You are an agent that can analyze the following data: ${JSON.stringify(data, null, 2)}. These data represent moments when the user felt nervous. The higher the count, the more nervous the user felt. Please provide a friendly, supportive analysis and advice on how to relieve stress.`
            }]
        },
        {
            role: "user",
    ...
    try {
        ...
        document.getElementById("geminiOutput").textContent = "Error: " + error.message;
    }
}
...
async function sendUserMessage() {
    ...
    document.getElementById("geminiOutput").innerHTML = `<b>You:</b> ${userText}<br><i>Gemini is thinking...</i>`;
    ...
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:streamGenerateContent?key=AIzaSyBaOQMQbeP-ACNwGlZ_XxTrbkhn1AZ2300`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload)
        });
        ...
        document.getElementById("geminiOutput").innerHTML = `<b>You:</b> ${userText}<br><b>Gemini:</b> ${reply.replace(/\n/g, "<br>")}`;
